...
void SequenceWindow::on_Button_InversesCount_clicked()
{
    int count = 0;
    if(ui->ChooseButton->currentIndex() == 0){
        count = InversionsCount(&MainArr);
    } else if (ui->ChooseButton->currentIndex() == 1) {
        count = InversionsCount(&MainList);
    }
    ui->Main_label->setText(QString::number(count));
}


void SequenceWindow::on_Button_SubSequences_clicked()
{

    Sequence<Sequence<long long int> *> *subsequences = nullptr;
    if (ui->ChooseButton->currentIndex() == 0) {
        subsequences = AllSubSequences(&MainArr);
    } else if (ui->ChooseButton->currentIndex() == 1) {
        subsequences = AllSubSequences(&MainList);
    }

    QString text;
    for(int i = 0; i != subsequences->GetSize(); ++i) {
        Sequence<long long int> *current_sequence = subsequences->Get(i);
        text += "[";
        for (int j = 0; j < current_sequence->GetSize(); ++j) {
            long long int current_value = current_sequence->Get(j);
            text += QString::number(current_value);
            if (j != current_sequence->GetSize() - 1) {
                text += ", ";
            }
        }
        text += "]\n";
    ...
    delete subsequences;
    QMessageBox::information(this, "AllSubSequences", text);
}
